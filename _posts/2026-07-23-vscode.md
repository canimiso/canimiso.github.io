...
![2026-07-23-124907](../assets/img/2026-07-23-124907.jpg){:width="80%"}

Image Pathを`/assets/img/`、Image Path Baseを`workspaceFolder`に変更しておいた。  
※画像内のassetsの綴りがasettsになっておりこれのせいで画像ファイルが無駄に増殖してびっくりした。

##### || 追記1

ご機嫌なMarkdown for Humans: WYSIWYG Editorですが、画像の貼り込みを`<img>`で行うと保存時丁寧にMarkdown形式に変換してくれます。設定にもこれを切る項目無いので困った。→今保存したらインラインコード指定忘れた文中のimgタグも変換して空欄になった。
...
Markdownで画像表示サイズ指定できればimgタグを使う必要がないので助かる。
...
左ペインのエクスプローラーへD&DすればOK(現環境なら`/assets/img/`)。  
